...
- Última tarea completada: verificación E2E de la FASE 6 en el emulador con notificación sintética real (ver sección 25) — motor + persistencia + servicio + UI funcionan juntos de principio a fin
- Tarea en curso: ninguna
- Próxima tarea: decidir con el usuario si se fusiona `claude/fase-3-mvp` a `main`, o continuar con más fases (recurrentes, dividendos en UI, importación CSV con UI real usando el parser de T-015, multidivisa real)
...
Qué funciona hoy, comprobado en la app: cuentas con saldo derivado, ingresos, gastos, transferencias (que conservan el patrimonio y no consumen presupuesto), categorías sembradas, presupuestos con umbral de aviso, carteras/activos/operaciones con coste medio ponderado y precios manuales, patrimonio neto con desgloses, dashboard con gráficos reales, tema oscuro por defecto y claro opcional, todo en español y sin conexión (la app ni siquiera declara permiso INTERNET). Persistencia Room v2 con esquema exportado; ajustes en DataStore. **Notificaciones bancarias (FASE 6) completa y verificada de extremo a extremo con una notificación sintética real** (sección 25): permiso voluntario del sistema, descubrimiento de apps sin `QUERY_ALL_PACKAGES`, autorización por app con cuenta asociada, propuestas pendientes con confirmar/descartar, movimiento real creado al confirmar.

Qué NO existe todavía: UI de importación/exportación y copias de seguridad (el parser CSV de Trade Republic ya existe en `data/importer/` desde T-015, pero sin pantalla), movimientos recurrentes, dividendos en UI, multidivisa real (hoy se excluyen las divisas distintas de la principal y el patrimonio se marca parcial), objetivos, cotizaciones externas y Supabase. No hay keystore de firma: solo APK de depuración.
...
## 23. T-017 — Primera vuelta rechazada por bug real de concurrencia (sesión 4)
- Codex entregó T-017 casi completa (135/154 tests, resto bloqueado por falta de red en su sandbox para Robolectric). Verificación de Claude con Gradle real (caché local de `android-all-instrumented` ya disponible de sesiones previas): 1 fallo real de aserción en `NotificationSettingsViewModelTest`.
...
## 25. FASE 6 verificada de extremo a extremo con notificación sintética (sesión 4)
- Emulador `mp_test` relanzado (headless, 1280 MB; el anterior murió sin dejar rastro en el log, probablemente por presión de memoria — no confirmado con certeza).
- Permiso de notificaciones concedido manualmente por el usuario simulado en los ajustes nativos de Android (`Settings.ACTION_NOTIFICATION_LISTENER_SETTINGS`); `dumpsys notification` confirma `BankNotificationListenerService` en «Live notification listeners».
- **Notificación sintética** enviada con `adb shell cmd notification post -S bigtext -t "BBVA" tag "Pago con tarjeta de 45,90 EUR en Supermercado Ejemplo"` (paquete real resultante: `com.android.shell`).
- **Trampa detectada y documentada**: al invocar `adb shell cmd notification post ...` desde PowerShell con argumentos separados, `adb` los reúne con espacios en una sola cadena para el shell remoto, que los vuelve a partir — el texto largo llega troceado (solo la primera palabra). Solución: pasar todo el comando remoto como una única cadena a `adb shell "..."`, con las comillas internas escapadas para que el shell remoto las vea. Mismo problema con `sqlite3` vía `run-as`: usar `adb shell "run-as ... sqlite3 ruta \"SELECT ...\""`. Además, Git Bash (MSYS) reescribe rutas `/data/...` como rutas de Windows salvo que se exporte `MSYS_NO_PATHCONV=1`.
- **Verificado en la base de datos real** (`run-as com.mipatrimonio.app sqlite3 .../mi_patrimonio.db`): tras autorizar «Shell» y asociar la cuenta, la segunda notificación (con el texto completo) generó una fila en `pending_proposal`: `GASTO, 4590 céntimos, EUR, "Supermercado Ejemplo", confianza ALTA, PENDIENTE`.
- **Verificado en la UI**: pantalla de propuestas pendientes la muestra correctamente («−45,90 €», comercio, fecha/hora real, «Confianza: alta»); al pulsar «Confirmar» desaparece de pendientes y aparece un movimiento real en Movimientos («Supermercado Ejemplo · Banco Principal · 22 sept 2026 · −45,90 €»).
- **Conclusión: FASE 6 (CLAUDE.md §13) implementada y verificada de extremo a extremo con una notificación sintética real**, no solo con tests unitarios. Cumple el requisito explícito de la sección 13 («Implementa pruebas con notificaciones sintéticas»).
- No probado en esta sesión: caso `TRANSFERENCIA` (solo descartar), revocar el permiso y comprobar que la pantalla lo refleja al volver, ni doble notificación idéntica dentro de la ventana de deduplicación de 5 min.

## 26. FEEDBACK DEL USUARIO TRAS PRUEBA MANUAL (sesión 5, 2026-09-23) — REGISTRADO, SIN EJECUTAR
Estado: PENDIENTE de planificar. Nada de esto está implementado todavía.
- F-01 (BUG, alta): la lectura de notificaciones no genera apuntes en el dispositivo real del usuario. Diagnóstico pendiente (¿app bancaria no descubierta/autorizada? ¿formato no reconocido por el parser? ¿servicio sin permiso o matado por el sistema?). El E2E solo se probó con notificación sintética en emulador.
- F-02: al comprar/vender un activo debe poderse elegir la cuenta de origen/destino del dinero (hoy falta la «cuenta de financiación», ya anotada como carencia de diseño en ROADMAP).
- F-03: Inicio y Patrimonio son casi idénticos; hay que diferenciarlos.
- F-04: patrimonio global con inversiones (a poder ser casi en tiempo real) y filtros para elegir qué se suma (cuentas corrientes, inversiones, etc.).
- F-05: subcategorías dentro de cada categoría (como Mi Presupuesto).
- F-06: crear cuentas y carteras desde el mismo menú, no por separado.
- F-07 (DIRECCIÓN DE PRODUCTO): la app son dos grandes bloques: (A) apuntes diarios, estadísticas, presupuestos (modelo Mi Presupuesto) y (B) tracker de activos: posiciones, gráfica por cartera y agregada (modelo getquin). Requiere rediseño de navegación/IA. El usuario ofrece los APK de ambas apps como referencia.
Próxima acción: acordar con el usuario el rediseño de navegación (F-07) y diagnosticar F-01 antes de fichas Codex.
- F-01 diagnóstico parcial (2026-09-23, solo lectura de código): el permiso y la autorización funcionan (la app se descubrió y se asoció cuenta). Causa más probable: `GenericSpanishParser` solo acepta textos con las palabras compra|pago|cargo|recibo|domiciliación (gasto) o ingreso|abono|nómina|devolución (ingreso) y un importe con €/EUR; no hay adaptador específico de Trade Republic. Si el texto real no las contiene, devuelve `NoInterpretable` y se descarta EN SILENCIO (sin rastro). También es posible que el texto esté en EXTRA_BIG_TEXT/EXTRA_SUB_TEXT y no en EXTRA_TEXT. Falta el texto real de una notificación de TR para confirmar. Acción: pedir el texto al usuario; ficha Codex = adaptador TR + leer BIG_TEXT + contador/diagnóstico de descartadas sin guardar texto sensible.
- D-008 (2026-09-23): decisión de producto para F-01: interpretación agnóstica del banco. Toda notificación de app autorizada con importe+divisa (sin contenido sensible) genera propuesta; tipo por heurística, por defecto GASTO/BAJA; el usuario corrige. Ficha `docs/tasks/T-018.md` escrita (estado pendiente, aún no lanzada a Codex). Sin cambios de esquema Room.
- D-009 (2026-09-23) PLANIFICADO, no ejecutado: «tipos de notificación» aprendidos y plantillas del usuario (idea del usuario). (1) Al ingerir una notificación se calcula su «esqueleto» (texto con importe y cifras sustituidos); si no existe plantilla para ese esqueleto, se crea como tipo nuevo sin clasificar y el usuario decide ingreso/gasto/transferencia al revisar la propuesta («recordar este tipo»). (2) Plantilla editable: texto ancla fijo (p. ej. «bizum de»), parte variable irrelevante, regla de signo (+ → ingreso, sin signo → gasto, o tipo fijo), importe extraído por el parser existente, cuenta/cartera asociada. (3) Transferencias cuenta→cartera de inversión: depende de la carencia F-02 (cuenta de financiación de las carteras), va DESPUÉS del modelo de datos. (4) Autoconfirmación: opt-in por plantilla, nunca por defecto (una notificación no garantiza operación definitiva, CLAUDE.md §13); movimientos marcados con origen NOTIFICACION y reversibles. Requiere tabla Room nueva → migración v3, riesgo alto. Orden: T-018 → modelo de datos (subcategorías + cuenta de origen inversiones) → T-019 plantillas → rediseño UI.
- Hallazgo (2026-09-23): las SUBCATEGORÍAS ya existen en modelo y UI (`parentId`, 2 niveles, selector de padre en el diálogo de categoría en Ajustes); F-05 es de descubribilidad/UX (dentro del rediseño), no de datos. Renumeración: T-019 = cuenta en operaciones de inversión (ficha escrita, pendiente, migración Room v3); plantillas de notificación (D-009) pasan a ser T-020. La aportación al bróker se modela como transferencia normal a una cuenta tipo INVERSION.
- T-018 ACEPTADA e integrada (2026-09-23): parser por importe + lectura de BIG_TEXT/SUB_TEXT + ignora resúmenes de grupo. Verificación de Claude: BUILD SUCCESSFUL (tests y assembleDebug). Falta prueba real con Trade Republic por el usuario (instalar el nuevo APK). Siguiente: lanzar T-019.
- Referencias de UI (2026-09-24): Aurora Store 4.8.4 (F-Droid, sha256 fd9c75d9…4d11) instalada en el emulador `mp_test` con login anónimo. getquin (`com.getquin.app`) instalada y arranca; requiere cuenta (la crea el usuario). Mi Presupuesto (`com.onetwoapps.mh`, 5,99 €, «My Budget Book» en inglés) es de pago: NO descargada (no se elude el pago); el usuario aportará capturas o la comprará. Navegación propuesta pendiente de confirmación del usuario (Inicio / Movimientos / Presupuestos / Inversiones / Patrimonio). T-019 sigue en curso en Codex (worktree t-019-cuenta-en-operaciones); Claude no debe tocar los archivos de ui/investments mientras tanto.
- Referencia Mi Presupuesto observada en el emulador (2026-09-24, app vacía, v9.13 «My Budget Book»): barra superior con 5 pestañas DESCRIPCIÓN GENERAL / TRANSACCIONES / ESTADÍSTICAS / DIAGRAMA / PRESUPUESTOS; selector mes+año con flechas y filtro; FAB «+»; barra inferior verde con «saldo actual» del periodo y de todas las cuentas; menú lateral: Gestión (cuentas, categorías, recurrentes, plantillas, presupuestos), funciones útiles (últimas entradas, búsqueda detallada, sin categoría), copia/restauración/exportar/importar CSV, ajustes. Categorías: pestañas Favoritas / Categorías / A-Z con buscador. getquin: el login por email no avanza en el emulador (sin rastro en logcat); pendiente. Las credenciales y el APK están en `datos-privados/referencias/` (ignorado por git; no copiar credenciales a ningún otro sitio).
- T-019: primera entrega de Codex revisada (build+tests OK en mi verificación en el worktree); rechazada parcialmente por COMISION inconsistente entre PositionCalculator (usa gross) y BalanceCalculator (usaba feesMinor) + cambio de diálogo no pedido. Devuelta con `docs/tasks/T-019.followup1.md`; el Codex anterior estaba colgado (proceso huérfano, detenido). Codex relanzado (tarea b4yo6k1n1).
- T-019 ACEPTADA e integrada (2026-09-24): cuenta de origen/destino en operaciones de inversión, Room v3 con migración 2→3, saldo derivado (COMPRA −(gross+fees), VENTA +(gross−fees), DIVIDENDO +(gross−ret.), COMISION −gross), solo misma divisa. 172 tests OK, BUILD SUCCESSFUL. Copia de seguridad de Mi Presupuesto (.mhs) aportada por el usuario en datos-privados/referencias/. Siguiente: T-020 (plantillas de notificación, Room v4) y rediseño de UI (bloques A/B); pendiente confirmación del usuario de la navegación.
- Hallazgo (2026-09-24): `canConfirm` bloquea las propuestas TRANSFERENCIA y las confirmadas nunca llevan categoría → un Bizum/plan de inversión quedaba atascado. T-020 (ficha escrita): confirmación editable (tipo, cuentas, categoría, comercio) + confirmar transferencias como `Transfer`. Renumeración: T-021 = plantillas de notificación aprendidas (Room v4, D-009).
- Referencia getquin observada en el emulador (2026-09-24; login por correo OK tras reescribir la contraseña carácter a carácter: `adb input text` con comillas había colado caracteres de más). Estructura (sin cifras del usuario): barra inferior Inicio / Patrimonio (Net Worth) / Analítica / Riqueza (Wealth) + Buscar, y botón «Importar». Inicio: patrimonio total con ocultar/actualizar, gráfica con rangos 1D 1W 1M YTD 1Y Max y comparación con benchmark. Patrimonio: sub-pestañas (Resumen / Inversiones / Efectivo…) y selector «Agregado» (cartera concreta o suma de todas) con ocultar/actualizar. Riqueza: progreso de inversión mensual con objetivo y Objetivos (jubilación…). Idea a adoptar: selector de cartera/agregado + rangos de tiempo + ocultar importes + benchmark opcional.
- T-020 ACEPTADA e integrada (2026-09-25): confirmación editable de propuestas (tipo, cuentas, categoría, comercio) y transferencias como `Transfer`. 175 tests OK. Excepción §34 documentada (2 líneas de test). NUEVO del usuario (2026-09-25): (a) la app debe ser la MISMA app que Mi Presupuesto + funciones de getquin + lectura/AUTOANOTACIÓN de notificaciones (no una mezcla); (b) fichas prototipo de UI en datos-privados/referencias/fichas-tarea-ui-mi-patrimonio.md (diseño Claude Design: Inicio/Movimientos/Presupuesto/Cartera/Más, tokens oscuros, Fraunces+DM Sans; su TICKET-04 usa mocks → NO usar mocks, datos reales de Room); (c) las notificaciones siguen sin registrarse con Trade Republic real tras T-018 (mismo comportamiento).
- T-022 (ficha escrita 2026-09-25, lanzada a Codex): diagnóstico de notificaciones (metadatos + motivo de descarte; texto solo con interruptor opt-in, 24 h), lectura ampliada (TEXT_LINES, MessagingStyle, ticker) y AUTOANOTACIÓN opt-in por app (OFF/SOLO_SEGURAS/TODAS; nunca transferencias; atómica). Room v4. Motivación: la lectura real de Trade Republic sigue sin generar apuntes tras T-018 y no podemos depurar en el móvil del usuario; además el usuario pide autoanotar.
- Referencia Mi Presupuesto con datos (2026-09-25, copia .mhs restaurada en el emulador; NO registrar cifras del usuario): Transacciones agrupadas por DÍA en tarjetas con «saldo diario» + fecha y, dentro, filas (título, «Subcategoría (Categoría), Cuenta», icono de recurrente, fecha, importe); barra inferior verde con saldo actual del periodo; el usuario ya usa categorías con subcategorías (p. ej. Periódicas dentro de Inversiones) y registra aportaciones a inversión como gastos categorizados. Diseño Claude Design del usuario leído (artifact 5aYmJ5FjAaRNK9AmosZiZQ, solo lectura): 5 pantallas 390x844 modo oscuro, tokens ya listados en las fichas; sus importes son de ejemplo (no usar). El usuario está rehaciendo el diseño: esperar su versión final antes de implementar UI; tokens/componentes base (TICKET-01/02) son estables.
- T-022 ACEPTADA e integrada (2026-09-25): diagnóstico de notificaciones, lectura ampliada, autoanotación por app; Room v4; 187 tests OK. DECISIONES DEL USUARIO (2026-09-25): (1) el desarrollo de la UI parte del PROTOTIPO Claude Design (se afinará juntos hasta la versión final); (2) navegación = pestañas INFERIORES (aceptada mi recomendación); (3) las notificaciones SÍ se leían pero quedaban como propuestas pendientes: el usuario quiere que se cree DIRECTAMENTE el apunte y corregirlo después → autoanotación activa por defecto en apps autorizadas (TODAS salvo transferencias); Bizum debe clasificarse por signo (+ ingreso; sin signo gasto), no como transferencia; (4) el usuario quiere replicar en lo posible el menú/formulario de CREAR APUNTE de Mi Presupuesto. Plan: T-023 (autoanotación por defecto + Bizum por signo), T-024 (base UI del prototipo: tokens, componentes, navegación inferior con datos reales), T-025 (formulario de apunte estilo Mi Presupuesto), T-026 (plantillas aprendidas, antes T-021).
- T-023 (ficha escrita 2026-09-25, lanzada): autoanotación TODAS por defecto al autorizar + migración 3→4 actualiza apps ya autorizadas; Bizum por signo (no transferencia).
- T-024 (ficha escrita 2026-09-25, pendiente; se lanza al aceptar T-023): base UI del prototipo (tokens, componentes, 5 pestañas inferiores Inicio/Movimientos/Presupuesto/Cartera/Más, pantalla Más con datos reales). Fuentes Fraunces y DM Sans (OFL) empaquetadas en res/font. Referencia del formulario de apunte de Mi Presupuesto en docs/ui-referencias/apunte-mi-presupuesto.md (base de T-025). Observación: el usuario modela su cartera de inversión como una CUENTA («Cartera») y las aportaciones como transferencias Trade Republic→Cartera.
- T-023 ACEPTADA e integrada (2026-09-25): autoanotación TODAS por defecto al autorizar (y migración para apps ya autorizadas), Bizum por signo. 191 tests OK. Excepción §34 documentada (helper de test). Limitación: OFF explícito se pierde al reautorizar. Siguiente: lanzar T-024 (base UI).
- T-024 ACEPTADA e integrada (2026-09-25): tema/tokens/fuentes, 9 componentes, navegación inferior Inicio/Movimientos/Presupuesto/Cartera/Más, pantalla Más. 195 tests OK; verificada en emulador incluyendo migración real 2→3→4. Siguiente: T-025 (formulario de apunte estilo Mi Presupuesto), luego rediseño pantalla a pantalla (Inicio, Movimientos, Presupuesto, Cartera) y T-026 plantillas.
- T-025 (ficha escrita 2026-09-25, lanzada): formulario unificado de apunte (Gasto/Ingreso/Transferencia) estilo Mi Presupuesto con selector de categoría a pantalla completa (Frecuentes derivadas, Categorías, A-Z); sin plantillas/recurrentes/recordatorio/foto.
- Diseño propuesta v2 (2026-09-25): lienzo NUEVO y privado https://claude.ai/artifact/B4dWtW5AYkSjwWuL2nikLA (el lienzo del usuario 5aYmJ5FjAaRNK9AmosZiZQ no se ha tocado): 8 pantallas con los tokens del prototipo (Inicio con filtros de patrimonio y aviso de apuntes automáticos, Movimientos por día con marca Auto, Nuevo apunte interactivo Gasto/Ingreso/Transferencia, selector de categoría, Estadísticas, Cartera con selector Agregado, detalle de activo con cuenta de origen, Notificaciones con registro automático). Datos de ejemplo solo de diseño. Generador en el scratchpad de sesión (no versionado). Pendiente: feedback del usuario antes de convertirlo en tickets de UI.
- T-025 ACEPTADA e integrada (2026-09-25): formulario unificado de apunte (ruta `apunte/{id}`) + selector de categoría a pantalla completa. 205 tests OK. Excepción §34 documentada (2 tests). E-010: los tests de ViewModel con stateIn(WhileSubscribed) son propensos a carreras si leen `uiState.value` o comprueban la BD antes de esperar al estado: usar siempre `uiState.first{}`. PULIDO PENDIENTE (T-026): hueco bajo la barra de estado en pantallas secundarias, estilo de campos, botón Guardar inferior, conforme a la propuesta v2. Cambio de numeración: plantillas de notificación → T-027.
- T-026 (ficha escrita 2026-09-25, lanzada): pulido visual (hueco superior de pantallas secundarias, formulario de apunte con filas etiqueta/valor, barra inferior Guardar, selector de categoría) sin cambios funcionales.
- T-026 ACEPTADA e integrada (2026-09-25): pulido visual del formulario y barra superior; 205 tests OK. El usuario da por BUENO el diseño v2 y pide implementarlo. Menor pendiente: signo del importe separado de la cifra en el formulario. Siguiente: T-027 (Inicio + Movimientos según diseño v2), luego Estadísticas/Presupuesto, Cartera + Activo, Notificaciones.
- T-027 (ficha escrita 2026-09-25, lanzada): Inicio (filtros de patrimonio Cuentas/Inversiones persistentes, ocultar importes, rangos 1M/3M/6M/1A/Máx, aviso de apuntes automáticos, Este mes) y Movimientos (por día con saldo del día sin transferencias, mes, segmentado, etiqueta Auto, filtro por origen) con datos reales. Sin Room.
- T-027 ACEPTADA e integrada (2026-09-25): Inicio y Movimientos v2 con datos reales; 214 tests OK. LECCIÓN E-011: Codex no puede compilar ni ejecutar tests en su sandbox, por eso entrega errores de sintaxis y tests que se cuelgan; los tests de ViewModel deben usar SIEMPRE `Dispatchers.setMain(UnconfinedTestDispatcher())` + `runTest` + `uiState.first{}` (nunca runBlocking). Añadir esto a COMMON.md para las próximas fichas. Pulido pendiente: plural de «apuntes creados», fondo de las filas, color del chip de variación, signo del importe en el formulario. Siguiente: T-028 (Presupuesto y Estadísticas).
- T-028 (ficha escrita 2026-09-25, lanzada): pestaña Presupuesto con segmentado Presupuestos/Estadísticas (Te quedan, progreso por categoría con umbral 90 %, anillo Top-5+Otros) y 4 detalles de pulido (plural, fondo de filas, chip de variación, signo del importe).
- Icono de la app (2026-09-25, EXCEPCIÓN §34: recurso pequeño pedido por el usuario, lo implementa Claude): fuente datos-privados/referencias/Icono_app.png (cartera verde, fondo transparente) → icono adaptativo (fondo marfil #F2F0E9, primer plano recortado al 62 % del lienzo, monocromo para iconos temáticos) en res/mipmap-*; manifiesto con icon y roundIcon. DECISIÓN D-010 (usuario): las carteras deben poder recibir dinero por transferencia desde una cuenta y computar en el patrimonio total; da igual si se clasifican como cartera o como cuenta → se mantiene el modelo actual (cuenta de inversión + transferencias + operaciones con cuenta de origen, T-019) sin doble conteo; el menú único de creación (cuenta/cartera) queda para una tarea de UI.
- T-028 ACEPTADA e integrada (2026-09-25): Presupuesto + Estadísticas v2 y 4 detalles de pulido; 218 tests OK. Corregidos por Claude título duplicado y barra de estado. LECCIÓN: toda pantalla con cabecera propia debe (a) añadirse a `hasOwnTopBar` en MiPatrimonioApp y (b) aplicar `statusBarsPadding()`; incluir en fichas de UI (Cartera, Más, Notificaciones). Siguiente: T-029 Cartera + detalle de activo.
- T-029 (ficha escrita 2026-09-25, lanzada): Cartera v2 (selector Agregado/cartera persistente, valor y rentabilidad simple, serie de valor, Posiciones/Distribución/Dividendos), detalle de activo, menú único Añadir cuenta/cartera. D-010 aplicada: sin cambio de modelo.
- BUG CRÍTICO (2026-09-26, reportado por el usuario): el selector de categoría no permitía elegir ninguna. Causa: regresión de T-026 en UnderlineTabs (Box con weight(1f) dentro de Column sin alto acotado → las pestañas ocupaban todo el alto y la lista medía 0). Fallo mío de revisión: al aceptar T-026 solo verifiqué el formulario, no el selector. Corrección directa de Claude (excepción §34, fallo crítico). LECCIÓN: al aceptar una tarea de UI, abrir en emulador TODAS las pantallas tocadas y pulsar sus controles principales.
- T-029 ACEPTADA e integrada (2026-09-26): Cartera v2, detalle de activo, menú único Añadir cuenta/cartera; 229 tests OK. Excepción §34 (1 import + 2 imports de test). BACKLOG DEL USUARIO (2026-09-26, mayoría inversión): (B1) operaciones con FECHA Y HORA/MINUTO (Room v5: epochDay→instante; afecta orden de PositionCalculator y precio de compra); (B2) autorrelleno por ISIN (nombre, ticker, mercado, precio en fecha/hora) consultando un proveedor en internet → requiere permiso INTERNET, elegir proveedor (verificar documentación, límites, licencias, claves; nada en el APK), interfaz abstracta + alternativa manual; es adelantar parte de la FASE 9; (B3) compra por IMPORTE: cantidad = (importe − comisión)/precio de mercado; (B4) gestión (listar/editar/eliminar/archivar) de CUENTAS y de ACTIVOS como ya existe para carteras; (B5) HECHO: «Comentario»→«Detalle» y selector de categoría; (B6) PRESUPUESTOS con mucho más detalle según Mi Presupuesto (capturar su configuración de presupuesto en el emulador). Más pendientes del usuario por llegar.
- T-030 (ficha escrita 2026-09-26, lanzada): gestión de cuentas (ver/editar/archivar/eliminar con reglas de integridad) y de activos, y compra/venta por importe (cantidad = (importe − comisión)/precio; función pura TradeSizing). Sin Room; si falta archived en AssetEntity, Codex eleva. INSTRUCCIÓN DEL USUARIO: mantener a Codex ocupado en paralelo lo antes posible; preparar la siguiente ficha mientras corre la anterior.
- T-031 (ficha escrita 2026-09-26, en cola tras T-030): presupuestos con detalle de Mi Presupuesto (nombre, importe, periodicidad, desde/hasta, categorías múltiples con subcategorías; Room v5 con migración 4→5, tabla budget_category). Observado en Mi Presupuesto: presupuesto = nombre + importe + rango de fechas + periodicidad + lista de categorías con subcategorías marcadas o (All). Emulador caído por falta de RAM (453 MB libres con Codex activo): relanzar solo entre tareas de Codex.
